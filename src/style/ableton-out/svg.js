...
return {
    ...
    createNode: function(node, render, description, icon) {
        ...
        nodeElm.append('path').attr('class', 'rpd-content')
                              ...
                              .attr('d', roundedRect(0, headerHeight, width, bodyHeight, 0, 0,
                                                        roundness, roundness));
        ...
        nodeElm.append('g').attr('class', 'rpd-process').attr('transform', 'translate(' + (pivot.x * width) + ','
        ...
        function checkNodeSize() {
            ...
            nodeElm.select('path.rpd-header').attr('d', roundedRect(0, 0, newSize.width, headerHeight,
                                                                    roundness, roundness, 0, 0));
            nodeElm.select('g.rpd-remove-button').attr('transform', 'translate(' + (newSize.width-12) + ',1)');
            nodeElm.select('path.rpd-shadow').attr('d', roundedRect(0, 0, newSize.width, newSize.height,
                                                                    roundness, roundness, roundness, roundness));
            ...
            nodeElm.select('path.rpd-content').attr('d', roundedRect(0, headerHeight,
                newSize.width, newSize.height - headerHeight,
                0, 0, roundness, roundness));
            nodeElm.select('g.rpd-process').attr('transform', 'translate(' + (pivot.x * newSize.width) + ','
                                                                           + (headerHeight + ((newSize.height - headerHeight) * pivot.y)) + ')');
            nodeElm.select('g.rpd-outlets').attr('transform', 'translate(' + newSize.width + ',' + headerHeight + ')');
            lastSize = newSize;
        }
